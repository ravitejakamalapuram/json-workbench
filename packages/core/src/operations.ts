...
function asArray(input: JsonValue): JsonValue[] {
  if (!Array.isArray(input)) throw new OperationError("Operation requires an array input");
  return input;
}
function asObject(input: JsonValue): JsonObject {
  if (input === null || Array.isArray(input) || typeof input !== "object") throw new OperationError("Operation requires an object input");
  return input;
}
    ...
    switch (definition.type) {
      case "pick": {
        ...
        return asArray(input).map((item) => {
          ...
          for (const key of selected) if (key in object) result[key] = object[key]!;
          return result;
        });
      }
      case "remove": {
        ...
        return asArray(input).map((item) => {
          const object = asObject(item);
          const result: JsonObject = {};
          for (const [key, value] of Object.entries(object)) if (!removed.has(key)) result[key] = value;
          return result;
        });
      }
      case "rename": {
        const from = definition.config.from;
        const to = definition.config.to;
      ...
      default:
        throw new OperationError(`Unsupported native operation: ${definition.type}`);
    }
  };
