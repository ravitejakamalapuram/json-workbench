...
import { inferJsonSchema, inferSchema } from "./schema";
...
describe("inferSchema", () => {
  it("infers nested object properties and array item types", async () => {
    const schema = await inferSchema(
      parseJsonStructure(
        chunks('{"users":[{"id":1,"name":"a"},{"id":2,"name":"b"}]}'),
      ),
    );
    ...
    expect(schema.properties?.users?.items?.properties?.id?.types).toEqual([
      "number",
    ]);
    expect(schema.properties?.users?.items?.properties?.name?.examples).toEqual(
      ["a", "b"],
    );
  });
  ...
  it("records heterogeneous types", async () => {
    const schema = await inferSchema(
      parseJsonStructure(chunks('[1,"two",null]')),
    );
  ...
  it("converts inferred structure into JSON Schema", async () => {
    const schema = await inferJsonSchema(
      parseJsonStructure(chunks('{"users":[{"name":"Ada"}]}')),
    );
    expect(schema.type).toBe("object");
    expect(schema.properties?.users?.type).toBe("array");
    expect(schema.properties?.users?.items?.properties?.name?.type).toBe(
      "string",
    );
  });
});
