...
describe("tokenizeJson", () => {
  ...
  it("preserves large integer lexemes", async () => {
    ...
    expect(result.find((token) => token.type === "number")?.value).toBe("90071992547409931234567890");
  });
  ...
  it("rejects unterminated strings", async () => {
    await expect(tokens(['{"name":"broken}'])).rejects.toMatchObject({ name: "JsonSyntaxError" });
  });
