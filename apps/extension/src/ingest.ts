...
export interface IngestProgress { readonly loaded: number; readonly total: number; }
...
export interface IngestTask { readonly cancel: () => void; }
...
export function ingestFile(file: File, handlers: IngestHandlers = {}): IngestTask {
  ...
  worker.onmessage = (message: MessageEvent) => {
    ...
    if (data.type === "event" && data.event) handlers.onEvent?.(data.event);
      ...
      settled = true;
      const error = Object.assign(new Error(data.message ?? "Failed to ingest JSON"), { name: data.name ?? "Error" });
      if (data.offset !== undefined) Object.assign(error, { offset: data.offset });
      handlers.onError?.(error);
      worker.terminate();
    }
  };
  worker.postMessage({ type: "ingest", file });
  return { cancel: () => { if (!settled) { settled = true; worker.terminate(); } } };
}
